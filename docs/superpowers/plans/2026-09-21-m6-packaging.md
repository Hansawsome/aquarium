...
**1. UBT `exit 65`의 최종 상태 — 원인 미특정, 우회 적용.** 진단은
[`docs/reviews/2026-09-21-m6-ubt.md`](../../reviews/2026-09-21-m6-ubt.md)에 있다. 명령·스킴·
pre-action 본문·환경 변수·cwd·stdin을 전부 기각했다 — 동일 명령이 `env -i`로도 셸에서는 성공하고
UBT 액션 실행기의 자식일 때만 실패한다. 유력한 남은 설명은 **Xcode 27.0이 UE 5.8의 문서화된
26.0 최소 / 26.1.1 권장 범위 밖**이라는 것이다. **Xcode 설치는 사용자 결정이므로 시도하지 않았다.**
`package_mac.sh`의 2단계 경로(`UE_BUILD_FROM_XCODE=1`로 receipt를 쓰게 한 뒤 PostBuildSync를 직접
실행)가 Development·Shipping 양쪽에서 동작하므로 패키징은 막히지 않는다. **알려진 이탈로 닫는다.**

**2. `TargetAllowList`는 의존 플러그인까지 실제로 막았다.** 게임 타깃 `Aquarium.rsp`의
`ModelContextProtocol` 참조가 6건 → **0건**, 같은 빌드의 에디터 메타데이터는 **16건**으로 유지됐다
(즉 플러그인은 살아 있고 게임에서만 빠졌다). `AllToolsets`도 함께 제한했다.

**3. Shipping 타깃 첫 컴파일에서 오류가 있었는가 — 있었다. 게임 타깃은 한 번도 컴파일된 적이 없었다.**
테스트 4파일이 `#if WITH_DEV_AUTOMATION_TESTS`만으로 가드돼 있었는데 그 매크로는 **게임 Development
빌드에서도 1**이라 에디터 전용 `UnrealEd` 헤더를 끌어왔다. M0의 "게임 타깃 컴파일 성공" 기록은 이
파일들이 생기기 전 시점의 것이다.

**4. 공식 성능 수치와 예비 레버.** 패키지 Development, 1920×1080, 30초 예열 + 180초, 2회:
**61.45 / 61.47 fps 평균, p95 16.58 ms 양쪽 동일**. 게이트 통과, 어느 실행도 60 fps 아래로 내려가지
않았다. **레버(`GridSizeZ` 128→64)는 쓰지 않았고 커튼·SSAO도 건드리지 않았다.**
**해석 주의:** 프레임의 73 %가 16.0~17.0 ms 한 구간에 몰려 있고 최소 프레임 시간이 3 ms다 —
**61.5 fps는 vsync 바닥이지 천장이 아니다.** "여유 1.5 fps"로 읽는 것은 틀렸고, 레버를 써도 이
숫자는 올라가지 않는다.

**5. 대조군 없이는 공허했을 뻔한 검사들 — 이 마일스톤 하나에서 네 건 나왔다.**

| 검사 | 계획서대로 했다면 | 실제로 무는 형태 |
|---|---|---|
| 개발 플래그 목록 추출 | `tr -d '-='`는 macOS `tr`이 옵션으로 파싱해 **출력이 비고**, `-Aquarium` 접두 패턴은 `AquariumNoFlee`를 놓친다 | 소스에서 `TEXT("...")`까지 포함해 뽑는다 — 실제 **11개** |
| MCP 배제 | "게임 빌드 로그 0건" — **대조군에서도 0건**(UBT는 플러그인 이름을 stdout에 찍지 않는다) | UBT 중간 산출물: `Aquarium.rsp` 6 → 0, 에디터 16 |
| 바이너리 문자열 스캔 | macOS `strings`에 **`-e` 플래그가 없어** UTF-16 `TEXT()` 리터럴이 보이지 않는다. Development 대조군에서도 0/11이 나와 Shipping의 0은 무의미했을 것이다 | `scripts/aq_scan.py`가 ascii·utf16le 양쪽을 스캔 |
| 패키지 머티리얼 컴파일 | `WorldGridMaterial\|Default Material` 단독 grep이 **멀쩡한 빌드의 `LogStreaming` 적재 안내에 걸려** 첫 실행부터 빨간불 | 실패 메시지로 좁히고, 좁힘이 눈멀게 하지 않도록 **`LogInit` 양성 대조**를 같은 실행에서 단언 |

**6. 실제로 물린 것을 확인한 검사들.** `font-licence`는 수정 전 Development·Shipping **양쪽 FAIL** →
`Content/Licenses` NonUFS 스테이징 후 양쪽 `ok`. 프라이버시 `auto` 모드는 별명을 임시로
`UE_LOG`하는 변이로 빨간불을 확인한 뒤 되돌렸다. `verify_package.sh`의 두 대조군(`control:mcp-scan`
7개 파일 적중, `control:config-scan`)은 매 실행에서 함께 단언된다.

**7. 물게 만들 수 없었던 것 — 하나 있다.** **Shipping 빌드의 머티리얼 컴파일 상태는 판정하지 못했다.**
Shipping은 로깅을 통째로 컴파일 아웃하므로 grep할 로그 자체가 존재하지 않는다. 우회로
"Development에서 0건이므로 Shipping도 0건"이라고 적을 수 있었지만 그것은 **다른 쿡 결과에 대한
추론**이지 측정이 아니다. **미판정으로 남긴다** — 사용자 수동 확인 3번이 이 항목을 덮는 유일한 수단이다.

**8. 사용자 수동 확인 인계 상태.** 5항목을 [`docs/reviews/2026-09-21-m6-summary.md`](../../reviews/2026-09-21-m6-summary.md)에
적어 넘겼다. 전부 **미수행**이며, 수행했다고 적힌 항목은 하나도 없다.

**9. 이월되는 항목(M6이 건드리지 않았다).** M4a~M5의 시각 항목(비늘 이방성, TubeCoral 실루엣이
말뚝 울타리처럼 읽히는 것, 부채 산호와 가지 산호의 구분, 동종 무리 육안 확정), 미사용
`SK_*_PhysicsAsset` 삭제 여부, 4K 품질 모드(SRS가 별도 검증이라고 적었다).
