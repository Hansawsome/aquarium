...
## 구현 중 발견한 후속 항목 (2026-09-20, 최종 리뷰)

- **소품 충돌 없음**: 물고기가 산호·바위를 통과한다. 소품 위치를 `avoid_distance`에 연결하는 것은 M4.
- **검증 구멍**: `verify_scene.py`가 소품 스케일과 소품 간 최소 간격을 검사하지 않는다(`REEF_WARN`도 계약에 없다). 다음 장면 작업 때 보강.
- **하드코딩된 `PROP_HALF_EXTENTS`**: 임포트 로그에서 베껴 온 값이라 메시가 바뀌면 조용히 어긋난다. 배치 시 `sm.get_bounds()`로 유도하는 편이 낫다.
- **레인 상수 중복**: `build_reef_m1.py`와 `verify_scene.py`가 같은 상수를 손으로 복사한다. 한쪽이 바뀌면 검증이 무의미해진다 — JSON 등으로 공유.
- **새 3종의 리그 값이 절대 cm**: `tail_tip_x` 등이 비율이 아니라 cm로 고정돼 몸길이를 바꾸면 꼬리 본이 지느러미에서 떨어진다.
- **`build_species` 키 검증 없음**: 오타 난 spec 키가 조용히 무시되고 기본값(블루탱 기준)이 쓰인다.
- **내보내기 성공 미확인**: `save_and_export`/산호 내보내기가 `bpy.ops` 결과를 확인하지 않아 실패해도 `_OK`가 찍힐 수 있다.
- **스펙 문구 불일치**: 스펙은 "소품 임포트 실패 시 건너뛰고 계속"이라 했으나 구현은 assert로 멈춘다(재현 스크립트에는 이쪽이 맞다). 스펙 문구를 고칠 것.
