...
- **포커스 델리게이트는 헤드리스에서 검증 불가** — F-06의 일시정지 로직 자체는 `SetPaused` 테스트로 고정했지만, 실제 윈도 포커스 상실/복귀 델리게이트는 `-nullrhi` 자동화에서 발생하지 않는다. 창 모드에서 다른 앱으로 전환했다가 돌아오는 수동 확인 1회가 필요하다.
- **유영 평면은 세션 시작 때 한 번만 적합된다** — 화면 비율에 맞춘 평면 재적합(F-07)은 세션 시작 시점의 요청 값으로 계산한다. 세션 도중 창 크기를 바꿔도 다시 적합되지 않는다. M4/M6에서 창 크기 변경 이벤트에 연결할지 결정한다.
- **벽을 따라갈 때 자세가 가파르게 선다** — 벽에 붙어 미끄러지는 동안 방향 벡터가 거의 수직이 되면서 물고기가 코를 위로 세운 자세로 보이는 구간이 있다(M3 검토 영상). 수직 전환 롤 문제와 같은 계열이므로 M4 실사 반복에서 함께 다룬다.
- **플레이어와 배경 물고기의 경계 동작이 의도적으로 다르다** — 배경 물고기는 `SteerAlongBoundary`(벽을 따라 미끄러짐, M1 방향 뒤집힘 해소), 플레이어 물고기는 `AvoidBoundary`(벽에서 정지, 요청하지 않은 표류 금지). 규칙 계층은 두 함수를 모두 유지하며, 선택은 `AFishActor::StepSwim`의 `bPlayerControlled` 분기에서 한다.
