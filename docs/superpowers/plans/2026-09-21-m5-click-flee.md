...
### 계획서가 틀렸던 곳 — 이번 마일스톤에서 가장 값진 기록

M5에서도 계획서의 테스트 다섯 개가 **물리적으로 불가능하거나 아무것도 검사하지 않았다.**
원인은 하나다: **`accel`이 30 cm/s²라서 180° 반전에 1.33초가 걸리는데 도망은 0.8초뿐이다.**
정면에서 클릭하면 도망 직후 속도는 **오히려 떨어진다**(실측 29.72 → 24.74 cm/s).

- **Task 3의 `flee is faster than cruising`, Task 4의 `flee wins over the held key`,
  Task 6의 `and it now goes the other way`** — 셋 다 위 이유로 **물리적으로 불가능**했다.
  더 강한 불변식으로 바꿨다: 속도를 순항 속도가 아니라 **`MaxSpeed`** 와 비교한다.
  `MaxSpeed`는 `StepMotion`이 잘라 내는 천장이므로 배율이 실제로 걸리지 않는 한 **도달할 수 없다.**
- **`FacingIsContinuousAcrossFlee`** — 계획서대로 쓰면 물고기가 **아예 방향을 틀지 않아서** 통과했다.
  에이전트가 "정말 반전했는가" 가드를 덧붙인 덕분에만 잡혔다. 워밍업을 줄여 반전이 도망 구간 **안에서**
  일어나게 고쳤다.
- **`FleeIncreasesTailAmplitude`** — 계획서대로는 속도를 올릴 수 없었다. **뒤에서** 클릭해
  도망 방향과 진행 방향이 같아지게 고쳤다.
- **`FleeDoesNotSlide`가 계획서대로는 아무것도 검사하지 않으면서 통과했다.** 정면 클릭은 물고기를
  직선으로 달리게 하므로 **미끄러질 일 자체가 없고**, 그래서 초록불이 켜졌다. 아래에서 클릭해
  **속도를 가진 채 90° 꺾이게** 고치고, "항로가 정말 10° 이상 바뀌었는가" 가드를 붙였으며,
  임계값을 계획서의 60°에서 **15°** 로 조였다.
- **계획서가 제안한 `FleeDoesNotSlide`용 변이는 물지 않았다.** 무는 변이를 따로 찾아
  15° 한계에 대해 **83.2°** 를 만들어 내는 것으로 교체해 확인했다.

**캡처 하네스(Task 9)에서도 계획서의 좌표 일곱 개가 전부 틀렸다.** `AUTO_CLICK` 기본값이 겨냥한
지점은 **7발 전부 빈 물**이었다(`CLICK_HITS=0`). 파서 경고는 0건이고 `armed 7`도 맞고 클립도 멀쩡했다 —
즉 **"적중이 하나도 없는데 완벽해 보이는 캡처"** 였고, 오직 적중/비적중 혼재 단언만이 그것을 잡았다.
M4c에서 지어낸 `-AquariumAutoInput` 토큰이 만들 뻔한 사고와 **정확히 같은 종류**다.
고친 방법: 클릭을 격자로 뿌리는 **프로브 캡처를 두 번** 돌려 `hit_plane_x`가 실제 평면 값을 돌려준
좌표만 골라 썼다. **물고기는 작고 움직이는 표적이므로 조준점은 기억으로 정할 수 없다**는 것을
이 스크립트 주석에 남겼다. 함께 두 번 조정한 것은 이것뿐이다(수치 재튜닝 2회, 구조 변경 없음).

또한 **`AUTO_INPUT`도 바꿨다.** 계획서는 t=16 근처에 `R6`을 눌러 두라고 했으나, 그러면 내 물고기가
30 cm/s²로 가속해 조준점에서 멀찍이 벗어난 뒤에 클릭이 날아간다. 키를 **클릭 0.2초 전**에 눌러
(`0 15.8,R1.0,0 13.2`) 키는 실제로 눌린 채이면서 물고기는 화면 중앙에 남게 했다.

### 헤드리스로는 검증할 수 없는 세 경로

- **역투영 경로(`DeprojectScreenPositionToWorld`)** — `-nullrhi`에는 게임 뷰포트가 없어 돌지 않는다.
  그래서 Automation 테스트는 전부 `HandleClickRay`에서 시작한다.
- **마우스 `BindKey` 바인딩** — 헤드리스에서는 입력이 발생하지 않는다.
- **HUD 나가기 버튼 호버 가드** — 헤드리스에서는 호버가 발생하지 않는다.

**이 셋의 유일한 방어선은 `-AquariumClickLog`의 CSV다.** 실제 실행에서 나온
`2026-09-21-m5-clicks.csv`는 7행 중 2행이 적중이고 `hit_plane_x`가 **592.6**(배경 평면)과
**220.0**(플레이어 평면)이다 — 둘 다 실제 존재하는 평면 값이므로 역투영이 진짜로 돌았다는 증거다.
`PropMaterialsCompile`이 셰이더 컴파일을 검사하지 못하는 것과 **같은 종류의 한계**이며,
`render_m5_click.sh`가 적중/비적중 혼재를 단언하는 이유도 이것이다.
**HUD 버튼 가드의 수동 확인은 아직 하지 않았다 — 사용자 확인 대기.**

### 클립에서 실제로 본 것

- **내 물고기(t=16.0, 평면 220)**: 클릭 직후 몸을 **점진적으로** 틀어 아래-오른쪽으로 확 나아가고,
  약 1.5초 뒤 바닥 근처에서 속도가 죽어 가라앉듯 정착한다. **순간 반전도 미끄러짐도 보이지 않았다**
  (F-13). 몸의 방향이 진행 방향과 계속 일치했다. 이것이 클립에서 가장 읽기 쉬운 증거다.
- **배경 물고기(t=5.2, 평면 592.6)**: 평면이 멀어 화면에서 매우 작게 그려진다.
  **어느 마리가 클릭당했는지 이 해상도에서는 눈으로 확정할 수 없다.** 적중 사실은 CSV로만 안다.
  솔직히 적는다 — 보지 못한 것을 봤다고 하지 않는다.
- **빈 바다 클릭(t=9.0, 12.0)과 빗나간 클릭(13.0, 13.2, 19.0)**: 배경 물고기 36마리가 원래
  자율적으로 움직이므로 **"아무 일도 안 일어났다"를 눈으로 증명하는 것은 불가능하다.**
  증거는 CSV의 `hit=0`과 `hit_plane_x=0.0`이다.
- **반복 클릭(t=5.6)**: 같은 화면 좌표를 0.4초 뒤에 다시 눌렀으나 **그 사이 물고기가 이미 도망쳐**
  빗나갔다(`hit=0`). 즉 **"도망 중 무시"를 클립으로 보이는 것은 고정 좌표로는 구조적으로 어렵다.**
  이 동작의 실제 보증은 Automation 테스트 쪽이다. 후속 항목으로 남긴다.
- **M4c 회귀 없음**: `-m5-compare.png`(왼쪽 M4c, 오른쪽 M5)에서 조명·안개·코스틱·산호·프롭 배치가
  모두 그대로다. 하네스가 hstack 양쪽을 원본과 PSNR로 대조해 **왼쪽이 정말 M4c 스틸인지**까지 단언한다
  (M4b에서 일괄 이름 변경이 마일스톤을 자기 자신과 비교하게 만든 사고가 있었다).

**`fleeSpeedScale`은 조정하지 않았다.** 2.2 그대로다 — 클립에서 도망이 충분히 "놀란 것처럼" 읽혔다.

### 성능

평균 **63.50 fps / p95 16.71 ms**(기본 2회 63.40 / 63.60), `-AquariumNoFlee`는 **63.26 fps**.
차이 **0.24 fps**는 노이즈 바닥 1.3 fps의 5분의 1이고 **부호까지 반대**이므로 어느 방향으로도 신호가 아니다.
M4c 기준선 63.46 대비 **+0.04 fps**. **`GridSizeZ` 128 → 64 레버는 쓰지 않았다** — 평균이 60 fps를
넘었고, 미리 쓰면 M6 쿡 빌드의 진짜 여유가 영구히 가려진다.

### 그대로 열려 있는 항목


- **F-06 창 포커스 동작은 사용자가 창 모드에서 alt-tab으로 1회 수동 확인해야 한다.** 헤드리스에서 포커스 델리게이트가 발생하지 않아 M3부터 이월된 항목이며 M5도 해결하지 못했다.
...
- **M5가 새로 여는 항목(설계 단계에서 이미 아는 것)**
  ...
  - **"도망 중 재클릭 무시"를 클립으로 보이지 못했다** — 고정 화면 좌표를 다시 눌러도 그 사이 물고기가 이미 이동해 빗나간다. Automation 테스트로만 보증된다.
  - **멀리 있는 평면의 물고기가 도망하는 모습은 1920×1080 클립에서 눈으로 확정할 수 없다** — 가까운 평면(플레이어)만 판독 가능하다.
