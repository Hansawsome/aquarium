...
### Blender — `assets/blender/BlueTang.blend`
...
- 규칙 계층 파일을 Unreal 빌드에 포함할 때 `std::function`·`<random>` 사용이 UBT 경고를 내면 경고 억제 대신 해당 파일을 `PrivateDefinitions` 없이 그대로 컴파일되도록 Build.cs에서 예외 규칙을 둔다. 규칙 코드를 Unreal용으로 복제·수정하지 않는다.
