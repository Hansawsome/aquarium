...
규칙 계층(C++17, 테스트 60개)과 Unreal 5.8 프로젝트(`unreal/Aquarium`, Automation 테스트 26개)가 있습니다. 한글 별명으로 입장하면 5종 중 한 마리가 무작위 배정되고, 카메라에 가장 가까운 평면에서 이름표를 달고 헤엄칩니다. 배경에는 5종 36마리가 서로 다른 깊이·크기로 유영하고 바닥에는 산호 3종과 바위가 놓여 있습니다(`docs/reviews/`에 검토 영상). 방향키 조종(M3), 실사 반복(M4), 클릭 도망(M5), 패키징(M6)은 아직입니다. 물고기·산호 모델은 1차 실루엣이라 실사 기준에는 못 미칩니다.
