#!/bin/bash
# M4a review artefacts: a 1 m close-up still of a single fish, the reef clip and
# scene still rendered with exactly the same arguments as render_m3_video.sh, and
# a side-by-side against the pre-M4a still shot with the same camera and seed.
#
# Outputs (docs/reviews/<date>-m4a-*):
#   closeup.png  one fish 1 m in front of the camera (eye, scales, fin membranes)
...
# It builds a throwaway level (/Game/Maps/_CloseupTmp) with editor Python: fog,
# sun and sky copied from the reef tuning, one FishActor pinned in place and a
...
#
# The close-up camera keeps the 1 m distance but uses a narrow FOV: at the
# reef's 75 deg the visible width at 1 m is ~1.5 m and a ~25 cm fish would be a
# sixth of the frame, which is useless for judging an eye or a scale row.
#
...
BEFORE="${BEFORE:-$REVIEWS/2026-09-21-m3-wall.png}"
CLOSEUP_SPECIES="${CLOSEUP_SPECIES:-BlueTang}"
CLOSEUP_FOV="${CLOSEUP_FOV:-30}"
DATE="$(date +%F)"
OUT="$REVIEWS/$DATE-m4a-reef.mp4"
TMP_MAP_PKG="$ROOT/unreal/Aquarium/Content/Maps/_CloseupTmp.umap"
...
if [[ "${SKIP_CLOSEUP:-0}" != "1" ]]; then
  PYDIR="$(mktemp -d)"; PY="$PYDIR/closeup_m4a.py"
  cat > "$PY" <<'PYEOF'
import os, unreal

SPECIES = os.environ.get("CLOSEUP_SPECIES", "BlueTang")
FOV = float(os.environ.get("CLOSEUP_FOV", "30"))
MAP = "/Game/Maps/_CloseupTmp"
Y = 5000.0            # far from the origin: the player fish spawns near X=220,Y=0
Z = 130.0
CAM_X, FISH_X = 0.0, 100.0   # 1 m in front of the camera
...
cam = spawn(unreal.CameraActor, (CAM_X, Y, Z), (0.0, 0.0), label="DiverCamera")
cam.tags = [unreal.Name("DiverCamera")]
cam.camera_component.set_field_of_view(FOV)

mesh_path = "/Game/Fish/%s/SK_%s" % (SPECIES, SPECIES)
...
fish = spawn(fish_cls, (FISH_X, Y, Z), (0.0, 90.0), label="CloseupFish")
fish.set_editor_property("fish_mesh", sk)
fish.set_editor_property("plane_origin", unreal.Vector(FISH_X, Y, Z))
# Pinned: a 1 cm plane and no speed keep the fish in the framing for the capture.
fish.set_editor_property("plane_half_width", 1.0)
fish.set_editor_property("plane_half_height", 1.0)
fish.set_editor_property("max_speed", 0.0)
fish.set_editor_property("seed", 1)
...
print("CLOSEUP_LEVEL_OK %s species=%s fov=%.1f" % (MAP, SPECIES, FOV))
PYEOF
  CLOSEUP_SPECIES="$CLOSEUP_SPECIES" CLOSEUP_FOV="$CLOSEUP_FOV" \
    "$UE_CMD" "$PROJ" -run=pythonscript -script="$PY" -unattended -nosplash \
    2>&1 | grep -E "CLOSEUP_LEVEL_OK|Error:|Fatal|AssertionError|Traceback" || true
  rm -rf "$PYDIR"

  run_capture "_CloseupTmp" 5
  CLOSEUP_FRAME="$(ls "$FRAMES"/UiFrame*.png | sed -n '90p')"   # ~t=3, after the auto nickname
  [[ -n "$CLOSEUP_FRAME" ]] || { echo "ERROR: no close-up frame" >&2; exit 1; }
  cp "$CLOSEUP_FRAME" "$REVIEWS/$DATE-m4a-closeup.png"
  echo "CLOSEUP_OK $REVIEWS/$DATE-m4a-closeup.png (from $(basename "$CLOSEUP_FRAME"))"
  rm -f "$TMP_MAP_PKG"
fi
...
if [[ "${SKIP_SCENE:-0}" != "1" ]]; then
  ...
  "$FFMPEG" -y -loglevel error -ss 12 -i "$OUT" -frames:v 1 "$REVIEWS/$DATE-m4a-scene.png"
  echo "SCENE_OK $REVIEWS/$DATE-m4a-scene.png"
fi

# --- Step 3: side by side ----------------------------------------------------
[[ -f "$BEFORE" ]] || { echo "ERROR: before image not found: $BEFORE" >&2; exit 1; }
"$FFMPEG" -y -loglevel error -i "$BEFORE" -i "$REVIEWS/$DATE-m4a-scene.png" \
  -filter_complex hstack "$REVIEWS/$DATE-m4a-compare.png"
echo "COMPARE_OK $REVIEWS/$DATE-m4a-compare.png (left=$(basename "$BEFORE"), right=$DATE-m4a-scene.png)"
